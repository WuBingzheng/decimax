...
fn mul_base() {
    ...
    let a = Dec::from_parts(1 << 60, 6);
    let b = Dec::from_parts(1 << 63, 8);
    let r = Dec::from_parts(((1 << 123) + 500) / 1000, 6 + 8 - 3);
    assert_eq!(a * b, r);

    // product of mantissas overflow
    let a = Dec::from_parts(1 << 90, 12);
    let b = Dec::from_parts(1 << 90, 14);
    let r = Dec::from_parts(153249554086588885835834702715030918, 12 + 14 - 19);
    assert_eq!(a * b, r);

    // overflow
    let a = Dec::from_parts(1 << 90, 12);
    let b = Dec::from_parts(1 << 90, 4);
